...
from message_platform_helper.rag import KnowledgeBase, KnowledgeBaseRetriever, build_rag_service, seed_default_knowledge
...
class ReasoningRagMemoryTests(unittest.TestCase):
    def test_reasoning_routes_channel_question_to_retrieval(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            memory = SQLiteMemoryStore(Path(tmp) / "memory.sqlite3").load("s1")
    ...
    def test_rag_ingest_and_search(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            kb = KnowledgeBase(Path(tmp) / "kb.sqlite3")
    ...
    def test_memory_updates_profile_and_facts(self) -> None:
        llm = RuleBasedLLMClient()
        with tempfile.TemporaryDirectory() as tmp:
            manager = MemoryManager(SQLiteMemoryStore(Path(tmp) / "memory.sqlite3"), llm)
